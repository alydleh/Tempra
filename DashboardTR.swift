...
import AWSIoT
import QuartzCore
class DashboardTR: UIViewController {
...
//
//            if globalVars.temps[3] >= ingredientsData.smokePoint(key!)
//            {
//
...
//
//
//            else if globalVars.temps[3] >= 100{
...
//        else if globalVars.temps[3] >= 100 //check that you don't have to check that a base hasn't been selected
//        {
//            burnerCircle.image = UIImage(named:"whitecircle.png")
//            //make the image white
//            burnerTest.updateBurner(temp,"None")
...
//    }
    ...
    func checkOil(){
        if BasesControllerTR.oilSelected.values.contains(true) == false {
            chosenOil = ""
...
//                print("chosen oil",chosenOil)
    ...
    func checkMeat(){
        if FoodControllerTR.meatSelected.values.contains(true) == false {
            chosenMeat = ""
        ...
        for (food, tf) in FoodControllerTR.meatSelected{
            //            print(food,tf)
...
//    @IBOutlet weak var tempBarTL: UIView!
    ...
    func updateTime()
    {
        self.timerLabel.text=SelectionScreenTR.timerTime
    }
    ...
    func setLabel(){
        ...
        if globalVars.temps[3] < 100
        {
            TopRightBurnerTemp.text="Off"
        }
            
        else if globalVars.temps[3] > 100 && BasesControllerTR.oilSelected.values.contains(true)  {
            
            
            if globalVars.temps.count>2{
                TopRightBurnerTemp.text=String(globalVars.temps[3])
//                TopRightBurnerTemp.sizeToFit()
            }
    ...
    @objc func doThings(){
        globalVars()
        setLabel()    // updates internal states
        switchState()
        ...
        checkMeat()
        checkOil()
        burnerUpdate(Float(globalVars.temps[3]),3)
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(TopRightBurnerTemp)
  ...
  //      _ = Timer.scheduledTimer(timeInterval: 0.5,target: self,selector: #selector(self.updateTime),userInfo: nil, repeats: true)
    ...
    func setupViews(maxTemp: Float) {
        ...
        var tempTR = Float(globalVars.temps[3] - 100) // Float(Server.temps[2]) //
        
        if tempTR < maxTemp {
            //print ("temp BR is less than max")
            var ratioTempTR = Float(tempTR/maxTemp)
            //print (ratioTempBR)
            ...
            progressViewTR.widthAnchor.constraint(equalToConstant: 130).isActive = true
            progressViewTR.heightAnchor.constraint(equalToConstant: 5).isActive = true
            //change the height
            progressViewTR.setProgress(ratioTempTR, animated: true)
            progressViewTR.progressTintColor = myColor
            
        } else {
            //print ("temp is greater than max 550")
            let ratioTempTR = Float(1.0)
            tempBarTR.addSubview(progressViewTR)
            progressViewTR.centerXAnchor.constraint(equalTo: tempBarTR.centerXAnchor).isActive = true
            progressViewTR.centerYAnchor.constraint(equalTo: tempBarTR.centerYAnchor).isActive = true
            progressViewTR.widthAnchor.constraint(equalToConstant: 130).isActive = true
