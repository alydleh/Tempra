...
import AWSIoT
import QuartzCore
class DashboardBL: UIViewController {
    ...
    func updateTime()
    {
        self.timerLabel.text=SelectionScreenBL.timerTime
    }
...
//
//            if globalVars.temps[0] >= ingredientsData.smokePoint(key!)
//            {
//
...
//
//
//            else if globalVars.temps[0] >= 100{
...
//        else if globalVars.temps[0] >= 100 //check that you don't have to check that a base hasn't been selected
//        {
//            burnerCircle.image = UIImage(named:"whitecircle.png")
//            //make the image white
//            burnerTest.updateBurner(temp, "None")
...
//
    ...
    func checkOil(){
        if BasesControllerBL.oilSelected.values.contains(true) == false {
            chosenOil = ""
...
//                print("chosen oil",chosenOil)
    ...
    func checkMeat(){
        if FoodControllerBL.meatSelected.values.contains(true) == false {
            chosenMeat = ""
        ...
        for (food, tf) in FoodControllerBL.meatSelected{
            //            print(food,tf)
...
//    @IBOutlet weak var tempBarBR: UIView!
    ...
    func setLabel(){
        ...
        if globalVars.temps[0] < 100
        {
            BottomLeftBurnerTemp.text="Off"
        }
            
        else if globalVars.temps[0] > 100 && BasesControllerBL.oilSelected.values.contains(true)  {
            
            if globalVars.temps.count>2{
                BottomLeftBurnerTemp.text=String(globalVars.temps[0])
//                BottomLeftBurnerTemp.sizeToFit()
            }
    ...
    @objc func doThings(){
        ...
        checkMeat()
        checkOil()
          burnerUpdate(Float(globalVars.temps[0]),0)
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(BottomLeftBurnerTemp)
  ...
  //       _ = Timer.scheduledTimer(timeInterval: 0.5,target: self,selector: #selector(self.updateTime),userInfo: nil, repeats: true)
    ...
    func setupViews(maxTemp: Float) {
        ...
        //arbitrary picking
        var tempBL = Float(globalVars.temps[0] - 100) // Float(Server.temps[2]) //
        ...
        if tempBL < maxTemp {
            ...
            var ratioTempBL = Float(tempBL/maxTemp)
            //print (ratioTempBR)
            ...
            progressViewBL.widthAnchor.constraint(equalToConstant: 130).isActive = true
            progressViewBL.heightAnchor.constraint(equalToConstant: 5).isActive = true
            //change the height
            progressViewBL.setProgress(ratioTempBL, animated: true)
            progressViewBL.progressTintColor = myColor
            
        } else {
            //print ("temp is greater than max 550")
            let ratioTempBL = Float(1.0)
            tempBarBL.addSubview(progressViewBL)
            progressViewBL.centerXAnchor.constraint(equalTo: tempBarBL.centerXAnchor).isActive = true
            progressViewBL.centerYAnchor.constraint(equalTo: tempBarBL.centerYAnchor).isActive = true
            progressViewBL.widthAnchor.constraint(equalToConstant: 130).isActive = true
